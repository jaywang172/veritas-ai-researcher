...
from workflows.enhanced_workflow import WorkflowError, create_enhanced_workflow
...
def print_header():
    ...
    print(
        "Multi-Round Reviews - Comprehensive Analysis - Actionable Results".center(70)
    )
    print("=" * 70 + "\n")
...
def save_result(content: str, goal: str, session_dir: Path = None) -> str:
    ...
    try:
        with open(filename, "w", encoding="utf-8") as f:
            ...
            f.write("# Process: Multi-round academic review with business insights\n")
            f.write("=" * 70 + "\n\n")
            f.write(content)
...
def main():
    ...
    # Optional data file
    data_file = input(
        "Data file for analysis (optional, press Enter to skip): "
    ).strip()
    if data_file and not Path(data_file).exists():
        print(
            f"WARNING: File '{data_file}' not found, proceeding with literature-only analysis"
        )
        data_file = None
    ...
    print("Expected deliverables: Comprehensive report + visualizations + insights")
    print("=" * 70)
    ...
    try:
        ...
        print("   - Data-driven analysis with visualizations")
        print("   - Strategic recommendations")
        print("   - Academic-quality citations and references")
        print("   - Multi-round peer review validation")
