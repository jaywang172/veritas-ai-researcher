...
class StepResult:
    ...
    def __init__(self, content: str, sources: List[str] = None, success: bool = True, error: str = None):
...
class Document:
    ...
    def get_content(self) -> str:
        return "\n\n".join(section.content for section in self.sections if section.is_valid())
    
    def has_data_analysis(self) -> bool:
        return any("數據分析" in section.content or "data analysis" in section.content.lower() 
...
class SimpleWorkflow:
    ...
    def __init__(self):
        ...
        from agents import (
            literature_scout, computational_scientist, 
            synthesizer, outline_planner, academic_writer, 
            editor, citation_formatter
        )
    ...
    def run(self, goal: str, data_file: str = None) -> Document:
        ...
        try:
            ...
            quality_result = self._quick_quality_check(final_result.content)
...
Output ONLY the JSON array, no other text.""",
            ...
            try:
                ...
                points = [{"point": result.raw, "source": "synthesis", "strength": "medium"}]
...
No JSON, just clear text outline.""",
    ...
    def _write_content(self, outline_result: StepResult, synthesis_result: StepResult) -> StepResult:
...
Return the complete paper with formatted references.""",
            ...
            if not result or not result.raw:
                ...
                print("Citation formatting failed, proceeding without formatted references")
                return StepResult(content=content)
            
            return StepResult(content=result.raw)
            
        except Exception as e:
            print(f"Citation formatting failed: {e}, proceeding without formatted references")
            return StepResult(content=content)
    ...
    def _quick_quality_check(self, content: str) -> StepResult:
        ...
        try:
            ...
            if word_count > 2000: score += 1
            if has_references: score += 1
            if has_abstract: score += 1
            if "NVIDIA" in content: score += 1  # Content relevance
