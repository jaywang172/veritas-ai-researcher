...
DOMAIN_CONFIGS = {
    ResearchDomain.BUSINESS: DomainConfig(
        ...
        data_analysis_focus=[
            "financial_metrics", "trend_analysis", "market_segmentation",
            "competitive_analysis", "roi_calculations", "growth_rates"
        ],
        visualization_types=[
            "line_charts", "bar_charts", "pie_charts", "scatter_plots",
            "heatmaps", "financial_dashboards"
        ],
        writing_style="executive_summary",
        citation_requirements="business_sources",
        quality_criteria=[
            "actionable_insights", "data_driven", "executive_friendly",
            "quantitative_support", "risk_assessment"
        ],
    ...
    ResearchDomain.ACADEMIC: DomainConfig(
        ...
        data_analysis_focus=[
            "statistical_analysis", "hypothesis_testing", "literature_synthesis",
            "methodology_validation", "sample_analysis"
        ],
        visualization_types=[
            "statistical_plots", "correlation_matrices", "distribution_plots",
            "box_plots", "regression_plots"
        ],
        writing_style="academic_formal",
        citation_requirements="apa_strict",
        quality_criteria=[
            "peer_review_ready", "methodology_sound", "literature_comprehensive",
            "statistical_validity", "reproducible"
        ],
    ...
    ResearchDomain.TECHNICAL: DomainConfig(
        ...
        data_analysis_focus=[
            "performance_metrics", "system_analysis", "benchmark_comparison",
            "error_analysis", "optimization_metrics"
        ],
        visualization_types=[
            "performance_charts", "system_diagrams", "benchmark_plots",
            "error_distributions", "network_graphs"
        ],
        writing_style="technical_documentation",
        citation_requirements="technical_sources",
        quality_criteria=[
            "technical_accuracy", "implementation_feasible", "performance_validated",
            "scalability_considered", "maintenance_addressed"
        ],
    ...
    ResearchDomain.SCIENTIFIC: DomainConfig(
        ...
        data_analysis_focus=[
            "experimental_analysis", "statistical_modeling", "data_validation",
            "uncertainty_quantification", "reproducibility_analysis"
        ],
        visualization_types=[
            "scientific_plots", "error_bars", "confidence_intervals",
            "publication_quality_figures", "data_distributions"
        ],
        writing_style="scientific_journal",
        citation_requirements="scientific_standards",
        quality_criteria=[
            "reproducible_results", "statistical_significance", "peer_reviewable",
            "methodology_transparent", "data_available"
        ],
    ...
    ResearchDomain.GENERAL: DomainConfig(
        ...
        data_analysis_focus=[
            "descriptive_statistics", "trend_analysis", "correlation_analysis",
            "basic_visualization", "summary_statistics"
        ],
        visualization_types=[
            "basic_charts", "trend_lines", "summary_plots", "simple_graphs"
        ],
        writing_style="professional_report",
        citation_requirements="standard_citations",
        quality_criteria=[
            "clear_communication", "evidence_based", "logical_structure",
            "appropriate_depth"
        ],
...
class DomainAdaptiveWorkflow(SimpleWorkflow):
    ...
    def auto_detect_domain(self, goal: str, data_file: str = None) -> ResearchDomain:
        ...
        # Business keywords
        business_keywords = [
            'revenue', 'profit', 'market', 'sales', 'financial', 'business',
            'strategy', 'competitive', 'roi', 'growth', 'investment', 'valuation'
        ]
        
        # Academic keywords
        academic_keywords = [
            'literature', 'review', 'theory', 'framework', 'hypothesis',
            'study', 'research', 'academic', 'scholarly', 'citation'
        ]
        
        # Technical keywords
        technical_keywords = [
            'system', 'performance', 'algorithm', 'implementation', 'architecture',
            'technical', 'engineering', 'software', 'hardware', 'optimization'
        ]
        
        # Scientific keywords
        scientific_keywords = [
            'experiment', 'data', 'analysis', 'statistical', 'methodology',
            'hypothesis', 'validation', 'scientific', 'empirical', 'measurement'
        ]
        ...
        if max_score >= 2:  # Confidence threshold
            ...
            print(f"Auto-detected domain: {detected_domain.value} (confidence: {max_score})")
...
Prioritize sources that match {self.config.writing_style} standards.""",
            ...
            if result and result.raw:
                ...
                return StepResult("Literature research failed", [], False, "No results from search")
    ...
    def _domain_data_analysis(self, data_file: str, goal: str) -> StepResult:
        ...
        try:
            ...
            analysis_config = {
                "focus_areas": self.config.data_analysis_focus,
                "visualization_types": self.config.visualization_types,
                "output_directory": str(self.domain_dir),
...
Use domain-appropriate analysis methods and generate professional visualizations.""",
            ...
            if result and result.raw:
                ...
                return StepResult("Data analysis failed", [], False, "No analysis results")
...
Ensure the output meets {self.domain.value} domain standards.""",
            ...
            if result and result.raw:
                ...
                return StepResult(content, [], True)  # Return original if rewriting fails
...
def create_domain_adaptive_workflow(domain: ResearchDomain = None) -> DomainAdaptiveWorkflow:
    """
...
def run_domain_research(goal: str, data_file: str = None, domain: ResearchDomain = None) -> str:
    """
    Execute domain-adaptive research workflow.
